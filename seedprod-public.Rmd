...
## Zero Inflation
To account for overdispersion, we switch from a Poisson distribution to a negative binomial.
...
All of these tests tell us that our data are likely to be zero-inflated and we will therefore use a zero-inflated GLMM.
...
The manipulation significantly reduces the abundance of bees in the community compared to the control.
...
There is no significant effect of the manipulation on the proportion of the *Bombus* community with long tongues.
...
modselect.tex %>%
  kable() %>%
  kable_styling("striped", full_width = F) 2%>% 
